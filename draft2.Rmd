...
```{r}
baterias_raw <- read_csv("site_root_cause.csv", lazy = FALSE, name_repair = make_clean_names)
```

```{r}
baterias <- baterias_raw |> 
...
```{r}
atr_raw <- read_csv("site_attribute.csv", lazy = FALSE, name_repair = make_clean_names)
```
...
tanque_baterias <- atr_raw |> 
 ...
 clean_names() |> 
 rename(siteid = cilocation,
        mtto = periodicidad_mg,
...
```{r}
raw_mg <- read_csv("site_mg_efficiency.csv", lazy = FALSE, name_repair = make_clean_names)
```
